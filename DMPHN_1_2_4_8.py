...
def save_deblur_images(images, iteration, epoch):
    filename = './checkpoints/' + METHOD + "/epoch" + str(epoch) + "_Iter_" + str(iteration) + "_deblur.png"
...
def main():
    ...
    for epoch in range(args.start_epoch, EPOCHS):
        ...
        train_dataset = GoProDataset(
            blur_image_files = './datas/256_data/train_blur_file.txt',
            sharp_image_files = './datas/256_data/train_sharp_file.txt',
            root_dir = './datas/256_data',
            crop = True,
        ...
        if (epoch)%100==0:
            ...
            test_dataset = GoProDataset(
                blur_image_files = './datas/256_data/test_blur_file.txt',
                sharp_image_files = './datas/256_data/test_sharp_file.txt',
                root_dir = './datas/256_data',
                transform = transforms.Compose([
                    transforms.ToTensor()
                ]))
        ...
        torch.save(encoder_lv1.state_dict(),str('./checkpoints/' + METHOD + "_new/encoder_lv1.pkl"))
        torch.save(encoder_lv2.state_dict(),str('./checkpoints/' + METHOD + "_new/encoder_lv2.pkl"))
        torch.save(encoder_lv3.state_dict(),str('./checkpoints/' + METHOD + "_new/encoder_lv3.pkl"))
        torch.save(encoder_lv4.state_dict(),str('./checkpoints/' + METHOD + "_new/encoder_lv4.pkl"))
        torch.save(decoder_lv1.state_dict(),str('./checkpoints/' + METHOD + "_new/decoder_lv1.pkl"))
        torch.save(decoder_lv2.state_dict(),str('./checkpoints/' + METHOD + "_new/decoder_lv2.pkl"))
        torch.save(decoder_lv3.state_dict(),str('./checkpoints/' + METHOD + "_new/decoder_lv3.pkl"))
        torch.save(decoder_lv4.state_dict(),str('./checkpoints/' + METHOD + "_new/decoder_lv4.pkl"))
