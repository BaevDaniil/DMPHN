...
class GoProDataset(Dataset):
    ...
    def __getitem__(self, idx):
        image_name_blur = self.blur_image_files[idx][0:-1].split('/')
        image_name_sharp = self.sharp_image_files[idx][0:-1].split('/')
        blur_image = Image.open(os.path.join(self.root_dir, image_name_blur[0], image_name_blur[1], image_name_blur[2], image_name_blur[3])).convert('RGB')
        sharp_image = Image.open(os.path.join(self.root_dir, image_name_sharp[0], image_name_sharp[1], image_name_sharp[2], image_name_sharp[3])).convert('RGB')
